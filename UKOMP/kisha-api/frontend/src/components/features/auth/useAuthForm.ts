...
/**
 * Shared hook for auth form state, error handling, and redirect logic.
 */
export function useAuthForm() {
  ...
  function handleError(submitError: unknown): void {
    if (
      ...
      const firstError = response.response?.data?.errors
        ? Object.values(response.response.data.errors)[0]?.[0]
        : null;
